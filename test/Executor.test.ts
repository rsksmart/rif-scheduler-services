...
describe('Executor', function (this: {
  ...
  test('Execute with low gasLimit should return ExecutionFailed state', async () => {
    ...
    const executeGas = (await this.web3.eth.getBlock('latest')).gasLimit;

    // this is needed because an issue with estimateGas in ganache
    (txExecutor as any).estimate = () => executeGas
